...
const RegisterPage: React.FC<RegisterPageProps> = ({ onRegister, onNavigateToLogin, isEditMode, uiTexts, onUpdateUiText, initialEmail }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.includes("Database error") || msg.includes("permission")) {
          msg = "Erro interno no banco de dados. Por favor, execute o script 'supabase_database_setup.sql' no Supabase SQL Editor para configurar as tabelas e permissões.";
      } else if (msg.includes("unique constraint") || msg.includes("duplicate key")) {
          msg = "Este e-mail ou nome de usuário já está cadastrado.";
      } else if (msg.includes("User already registered")) {
          msg = "Este e-mail já está cadastrado. Tente fazer login ou recuperar sua senha.";
      }
  ...
  return (
    <div className="min-h-screen w-full bg-black flex items-start justify-center p-4 pt-12 relative overflow-hidden">
       {/* Video Background */}
      ...
      >
        <div className="text-center mb-6 w-full">
            <div className="relative z-10">
                ...
                <p className="text-gray-400 text-xs mt-1 uppercase tracking-widest">Transforme seu corpo hoje</p>
            </div>
        </div>
          ...
          <div className={`flex items-start gap-3 p-4 rounded-xl border-2 transition-all duration-300 shadow-lg mt-4 mb-2 ${
              ...
              : 'bg-[#2e1065] border-[#581c87] hover:border-[#7c3aed]' // Roxo muito escuro quando não selecionado
                ...
                <label htmlFor="legal" className="text-[11px] text-white leading-relaxed select-none cursor-pointer font-medium">
                    Declaro estar de acordo com o exposto nos{' '}
                    <button 
                        type="button"
                        onClick={(e) => { e.preventDefault(); setViewingDoc('terms'); }} 
                        className="text-blue-300 font-bold hover:text-white hover:underline cursor-pointer transition-colors bg-transparent border-none p-0 appearance-none inline uppercase text-[10px]"
                    >
                        Termos de Uso
                    </button>
                    {' '}e{' '}
