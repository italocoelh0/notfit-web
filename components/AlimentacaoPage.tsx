                    ...
                    {isAdmin ? (
                        <>
                           ...
                           <div className="flex items-center gap-2 mb-4">
                               ...
                               <input type="checkbox" checked={!!editedRecipe.isElite} onChange={e => handleInputChange('isElite', e.target.checked as any)} className="w-5 h-5 accent-primary" />
                           </div>
                        ...
                        <>
                            <div className="flex flex-col items-center text-center mb-8">
                                ...
                                <div className="flex gap-2 justify-center">
                                    ...
                                    {editedRecipe.isElite && (
                                         <div className="inline-block px-3 py-1 rounded-full border border-yellow-500/30 bg-yellow-500/10 text-yellow-500 text-xs font-bold uppercase tracking-widest">
                                            <i className="fa-solid fa-crown mr-1"></i> Elite
                                        </div>
                                    )}
                                </div>
...
const RecipeListItem: React.FC<{
  ...
  canAccess: boolean;
}> = ({ recipe, isAdded, onToggleAdd, onViewDetails, canAccess }) => {
  return (
    <motion.div
      layout
      ...
      viewport={{ once: true }}
      className="bg-black/40 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden group hover:bg-white/5 hover:border-white/20 transition-all duration-300 relative"
    >
      {!canAccess && (
          <div className="absolute inset-0 bg-black/70 backdrop-blur-md z-20 flex flex-col items-center justify-center p-4 text-center">
              <i className="fa-solid fa-lock text-primary text-2xl mb-2"></i>
              <p className="font-anton text-white uppercase text-xs tracking-widest">Conteúdo Elite</p>
              <p className="text-[10px] text-gray-400 mt-1">Faça o upgrade para acessar</p>
          </div>
      )}
...
const AlimentacaoPage: React.FC<AlimentacaoPageProps> = ({ recipes, userData, onUpdateRecipe, onAddRecipe, onDeleteRecipe, isEditMode, uiTexts, onUpdateUiText, onUpdateUserData, onSendNotification, setActiveSection }) => {
    ...
    const handleToggleRecipe = (recipe: Recipe) => {
        if (recipe.isElite && !canAccessElite) return;
    ...
    return (
        <div className="pb-32">
            <div className="sticky top-16 z-30 bg-background/90 backdrop-blur-md -mx-4 px-4 py-3 flex items-center gap-3 border-b border-white/10">
                <button onClick={() => setActiveSection('feed')} className="w-8 h-8 flex items-center justify-center bg-surface-100 rounded-full text-white hover:bg-primary hover:text-white transition-colors">
                ...
                <AnimatePresence>
                    {filteredRecipes.map(recipe => (
                        <RecipeListItem 
                            key={recipe.id} 
                            recipe={recipe} 
                            canAccess={!recipe.isElite || canAccessElite}
                                ...
                                {addedRecipes.map(recipe => (
                                    <div key={recipe.id} className="bg-black/40 p-4 rounded-xl border border-white/5 flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 rounded-lg bg-surface-200 flex items-center justify-center text-2xl">{recipe.icon}</div>
                                            <div>
                                        ...
                                        <button onClick={() => handleToggleRecipe(recipe)} className="text-red-500 w-8 h-8 flex items-center justify-center"><i className="fa-solid fa-times"></i></button>
                                    </div>
                                ))}
                            </div>
                             <div className="p-6 mt-auto border-t border-white/10 bg-black/20 safe-area-bottom">
                                <button onClick={handleAddToRoutine} className="w-full bg-primary text-white font-anton uppercase tracking-widest text-sm py-4 rounded-xl">Criar Plano</button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
