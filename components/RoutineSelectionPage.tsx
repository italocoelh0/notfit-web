...
import { UITexts, DashboardSection, UserData } from '../types';
...
interface RoutineSelectionPageProps {
  ...
  userData: UserData; // Alterado de isAdmin para userData completo
}
...
const RoutineSelectionPage: React.FC<RoutineSelectionPageProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[50] w-full h-full bg-black flex flex-col items-center justify-center overflow-hidden pb-20 pt-16 px-4">
       {/* Back Header */}
       <div className="absolute top-0 left-0 w-full z-[60] px-4 py-4 flex items-center gap-3 bg-gradient-to-b from-black/90 to-transparent pt-6 pt-safe">
            <button 
                onClick={() => setActiveSection('feed')} 
                className="w-10 h-10 flex items-center justify-center bg-surface-200 rounded-full text-white hover:bg-primary hover:text-white transition-colors border border-white/10 active:scale-90"
            >
                <i className="fa-solid fa-arrow-left"></i>
            </button>
            <span className="font-anton uppercase tracking-wider text-lg text-white">Central</span>
       </div>
       ...
       <div className="relative z-10 w-full max-w-md flex flex-col h-full justify-center mt-8 overflow-y-auto hide-scrollbar">
            ...
            <div className="space-y-4 w-full px-2 pb-10">
                ...
                {/* SOMENTE ADMIN PODE EDITAR GALERIA DE DADOS */}
                {isAdmin && (
                    <HubCard 
                        onClick={() => setActiveSection('content_manager' as any)} 
                        icon="⚙️"
                        title="Gerenciador"
                        subtitle="Editar exercícios e vídeos (Admin)"
                        admin={true}
                    />
                )}
            </div>
