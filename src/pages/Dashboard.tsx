...
// FIX: Ensure correct named import
...
const Dashboard: React.FC<DashboardProps> = props => {
  ...
  // Helper to group messages into conversations (mock implementation logic)
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case 'notifications':
        return (
          <NotificationsPage
            notifications={notifications.filter(n => n.fromUserId !== userData.id)}
      ...
      case 'events': 
        return (
          <SocialPage
            ...
            allUsers={allUsers} // Pass allUsers for searching members
          />
        );
      case 'rotina':
        if (!isSubscriptionWallPassed) {
          return <SubscriptionPage onSkip={() => setIsSubscriptionWallPassed(true)} />;
        }
        return (
          <RoutinePage
            userData={userData}
      ...
      case 'editProfile':
        return (
          <EditProfilePage
            userData={userData} onUpdateUserData={onUpdateUserData} onLogout={onLogout}
            onBack={() => setActiveSection('profile')} isEditMode={isEditMode} uiTexts={uiTexts}
            onUpdateUiText={onUpdateUiText}
          />
        );
      case 'adminPanel':
      ...
      case 'producerPanel':
        return (
            <ProducerPanel 
                ...
                allUsers={allUsers} 
                onNavigateToChat={onNavigateToChat}
                onUpdateOtherUser={onUpdateOtherUser}
            />
        );
      case 'custom_routine':
        return (
            <StudentCustomRoutinePage 
                userData={userData}
                onUpdateUserData={onUpdateUserData}
                isEditMode={isEditMode}
                uiTexts={uiTexts}
    ...
    <div className="min-h-screen bg-background pb-safe">
      <header className="fixed top-0 left-0 right-0 z-10 bg-gradient-to-b from-black/90 to-black/0 backdrop-blur-sm pt-safe">
        <div className="container mx-auto px-4 h-16 grid grid-cols-3 items-center">
          ...
          {/* Left: Back Button (if Profile/Stats) OR Avatar (User Menu) */}
          <div className="justify-self-start">
            {activeSection === 'profile' ? (
                <button 
                    onClick={() => setActiveSection('feed')}
                    className="w-10 h-10 rounded-full bg-surface-200 border border-white/10 overflow-hidden flex items-center justify-center hover:border-primary transition-colors shadow-lg text-white"
                >
                ...
                <button 
                  ...
                  className="w-10 h-10 rounded-full bg-surface-200 border border-white/10 overflow-hidden flex items-center justify-center hover:border-primary transition-colors shadow-lg"
                >
          ...
          <div className="justify-self-center">
            <button 
                ...
                className="flex items-center gap-2 bg-gradient-to-r from-orange-600 to-orange-500 text-white px-5 py-1.5 rounded-full text-[11px] font-anton uppercase tracking-widest hover:scale-105 transition-transform shadow-lg shadow-orange-500/20 border border-orange-400/30"
            >
          ...
          <div className="justify-self-end flex items-center gap-3">
             {/* Flames Direct Link */}
             <button 
                onClick={() => setActiveSection('flamesStore')}
            ...
            <button 
              ...
              className={`text-xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-black/20 transition-colors ${activeSection === 'messages' ? 'text-primary' : 'text-on-surface-secondary hover:text-on-surface'}`}
            >
            ...
            <button
              ...
              className={`text-xl w-8 h-8 flex items-center justify-center rounded-full hover:bg-black/20 transition-colors ${activeSection === 'notifications' ? 'text-primary' : 'text-on-surface-secondary hover:text-on-surface'}`}
            >
      ...
      {/* Main Content - Adjusted top padding to account for safe area + header height */}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-surface-100/90 backdrop-blur-lg border-t border-surface-200 flex justify-around items-center z-20 pb-safe pt-2">
        <NavItem
          iconClass="fa-solid fa-users" label="Feed" isActive={activeSection === 'feed'}
          onClick={() => setActiveSection('feed')}
        />
        <NavItem
          iconClass="fa-solid fa-calendar-days" label="Rotina" isActive={['rotina', 'alimentacao', 'treinos', 'consultancy', 'producerPanel', 'custom_routine'].includes(activeSection)}
          onClick={() => setActiveSection('rotina')}
        />
        <button
          onClick={() => setIsActivityTrackerOpen(true)}
          className="w-16 h-16 bg-primary rounded-full flex items-center justify-center text-white text-3xl shadow-[0_0_20px_rgba(252,82,0,0.5)] -mt-8 transform hover:scale-110 transition-transform mb-2"
          aria-label="Registrar Atividade"
        >
            <i className="fa-solid fa-fire"></i>
        </button>
        <NavItem
          iconClass="fa-solid fa-user-group" label="Social" isActive={activeSection === 'events'}
          onClick={() => setActiveSection('events')}
        />
        {/* The 'Você' tab points to Stats */}
        <NavItem
          iconClass="fa-solid fa-chart-pie" label="Você" isActive={activeSection === 'profile'}
      ...
      <AnimatePresence>
        ...
        {isSubscriptionModalOpen && (
            <SubscriptionPage onSkip={() => setIsSubscriptionModalOpen(false)} />
        )}
      </AnimatePresence>
