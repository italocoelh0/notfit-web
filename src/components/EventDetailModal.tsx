                            ...
                            {isLoading ? <p className="text-sm text-center text-on-surface-secondary">Carregando...</p> : comments.length > 0 ? comments.map(c => (
                                <div key={c.id} className="flex items-start gap-2">
                                    <div className="w-8 h-8 bg-surface-200 rounded-full flex-shrink-0 overflow-hidden">
                                        {c.profile?.user_avatar && c.profile.user_avatar.startsWith('data:image') ? <img src={c.profile.user_avatar} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-sm">{c.profile?.user_avatar || '👤'}</div>}
                                    </div>
